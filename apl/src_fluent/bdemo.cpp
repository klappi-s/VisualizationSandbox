...
int main(){
    ...
    // Demo: Rebind same variable name with VIS_REBIND
    {
        ...
        std::cout << "VIS_REBIND density: " << vis.get(id<"density">).data << "\n";
        std::cout << "VIS_REBIND phi: " << vis.get(id<"phi">).data << "\n";
    ...
    // Demo: Continuation-style with_added helper
    {
        ...
        with_added<"density">(std::move(vis), fx, [&](auto&& vis_after){
            std::cout << "with_added density: " << vis_after.get(id<"density">).data << "\n";
        });
    }
